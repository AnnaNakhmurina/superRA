...
| **refactor-and-integrate** | Post-sync integration gates — drift-test quality, codebase fit, project-doc audit, task-local Sync impact propagation, and minimum surviving branch delta — shared by implementer and reviewer. Use during Integrate, or standalone for any refactor that needs consistent quality gates. |
| **semantic-merge** | Intent-based semantic sync with mode references for workflow sync authoring, sync review, and standalone merge. It classifies conflicts by role (behavior/API, data/schema, docs, generated outputs, tests, config/build), escalates intent-changing decisions to the user, lands exactly one minimal merge commit that leaves existing protection passing, and records deferred propagation in branch-level Sync Map + task-local or file-local impact notes for `refactor-and-integrate` to satisfy. Use whenever you would otherwise run `git merge` / `git rebase` / `git cherry-pick` — the `merge-guard` hook flags bare invocations automatically. |
