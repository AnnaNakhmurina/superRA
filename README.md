...
| **refactor-and-integrate** | Tools for **codebase coherence** — convention fit, utility reuse, PR-friendly diffs, Project Doc Audit walk-up, minimum net diff against the host — plus drift-test quality and task-local Sync impact propagation. Shared by implementer and reviewer. Use during Integrate, or standalone for any refactor that needs consistent quality gates. |
| **semantic-merge** | Tools for **semantic coherence** in branch integration — intent investigation, role classification (behavior/API, data/schema, docs, generated outputs, tests, config/build), conflict resolution, intent-changing escalation, stale-reference detect-and-resolve, and propagation-to-coherence. Lands a merge commit plus propagation commits as needed to reach semantic coherence, with every commit leaving existing protection passing, and records deferred codebase-coherence work in branch-level Sync Map + task-local or file-local impact notes for `refactor-and-integrate` to satisfy. Use whenever you would otherwise run `git merge` / `git rebase` / `git cherry-pick` — the `merge-guard` hook flags bare invocations automatically. |
