...
| `refactor-and-integrate` | Drift-test quality, post-sync codebase integration, project-doc audit, task-local Sync impact propagation, and minimum surviving branch delta. Loaded by integration-phase agents. |
...
| `semantic-merge` | Intent-based semantic sync for any vertical or caller. Provides mode references for workflow sync authoring, workflow sync review, and standalone merge; resolves conflicts by intent, escalates intent-changing decisions to the user, lands exactly one minimal merge commit that leaves existing protection passing, and records deferred propagation in branch-level Sync Map plus task-local or file-local impact notes for `refactor-and-integrate` or the caller to satisfy. |
