...
> 1. [MAJOR] `skills/using-superRA/references/direct-mode-implementer.md:94` and `:131` leak subagent-dispatch-only wording into direct mode. Line 15 correctly says "In direct mode there is no dispatch prompt," yet line 94 reads "On a first dispatch there is no review-notes blockquote yet ... Your re-dispatch prompt carries a one-line delta pointing at what changed," and line 131 reads "**Parallel worktree dispatch (`Worktree:` field set).** Return the `<branch>/parallel/<slug>` branch name and HEAD SHA in your status report ...". Neither concept exists in direct mode. RESULTS.md claims the old `cleanup_implementer_handoff`'s targets "no longer exist in source," but the equivalent re-dispatch paragraph is still at `agents/implementer.md:97` and the Worktree block is still at `agents/implementer.md:134` (just reworded from "If the dispatch included a `Worktree:` field" to "**Parallel worktree dispatch (`Worktree:` field set).**"). Pre-audit at `34bf20d` the cleanup stripped both for direct mode. Fix: restore the equivalent direct-mode cleanup in `skills/codex-superra-setup/scripts/sync_codex_agents.py` — rewrite "first dispatch" / "re-dispatch prompt ..." to direct-mode wording (e.g., "On a first pass" and drop the re-dispatch-delta sentence), and strip the Worktree paragraph entirely. Then regenerate `direct-mode-implementer.md` and the Codex `.toml` mirrors.
>    → implemented: added `cleanup_implementer_handoff` in `skills/codex-superra-setup/scripts/sync_codex_agents.py:321`; rewrote the §How You Fix Review Items opener to "On a first pass ..." with the re-dispatch-delta sentence dropped, and stripped the Parallel worktree dispatch paragraph entirely. Regenerated `direct-mode-implementer.md` (now at `:94` and no `:131`) — leak grep clean.
> 2. [MAJOR] `skills/using-superRA/references/direct-mode-reviewer.md:104` leaks "The orchestrator passes the round context (`origin/<base-branch>`, `MERGE_BASE_SHA`, reviewed upstream range, and any special steering) in the dispatch." There is no dispatch in direct mode. Pre-audit at `34bf20d` the removed `cleanup_reviewer_handoff` rewrote this to "Use the current round context ... from `PLAN.md` / the current session when the base-side scan surfaces material overlap, and create or update the section for the current round from that context." That rewrite was dropped when the cleanup helper was deleted on the grounds its targets "no longer exist in source," but the subagent source at `agents/reviewer.md:109` was rewritten, not deleted, and now its dispatch-only wording leaks into direct mode. Fix: restore the equivalent rewrite in `sync_codex_agents.py` and regenerate.
>    → implemented: added `cleanup_reviewer_handoff` in `skills/codex-superra-setup/scripts/sync_codex_agents.py:363`; rewrote the `## Upstream Intent` ownership paragraph in `direct-mode-reviewer.md:104` to source the round context from `PLAN.md` / the current session and removed the "Implementers are hands-off" and "the orchestrator removes it in the Phase B closeout commit" dispatch-implied phrasing. Both cleanup helpers raise `ValueError` if their expected source strings disappear so future regenerator drift becomes a test failure.
> 3. [MAJOR] `skills/agent-orchestration/references/worktree-harness-fallback.md:30` now has a broken pointer: "Agents dispatched with a `Worktree:` field are instructed to do this themselves (see `agents/implementer.md` §Before You Start)." The implementer's §Before You Start no longer contains the Worktree-enter step (it was deleted as Anti-Pattern #5 in the survey). The dispatch's `Additionally:` line carries the behavior, but the cross-reference in worktree-harness-fallback.md is now stale. Fix: either repoint the reference at `agent-orchestration/SKILL.md §Parallelization` (where the required `Additionally:` steering is specified at lines 91-93) or drop the sentence since agent-orchestration's §Parallelization is already the authoritative source.
>    → implemented: repointed the cross-reference at `skills/agent-orchestration/references/worktree-harness-fallback.md:30` to `SKILL.md §Parallelization and Worktree Isolation` and stated the behavior comes from the dispatch's `Additionally:` steering (the authoritative source).
> 4. [MINOR] `RESULTS.md` Task 6 "Verification" bullet claims "every POINTER inserted this round points at content that actually exists at the cited source — ... `econ-data-analysis` (per `econ-data-analysis/SKILL.md §Discipline`) carries Data-First principles." `econ-data-analysis/SKILL.md` has no `§Discipline` heading; the Data-First content lives under `## The Iron Law` and the `[BLOCKING]` / `[ADVISORY]` gated checklists. Fix: update the RESULTS.md reference to the actual section name(s) or drop the "§Discipline" anchor. The pointer target exists; only the cited anchor is wrong.
>    → implemented: updated RESULTS.md Task 6 Verification bullet — replaced `§Discipline` anchor with the correct "§The Iron Law and the `[BLOCKING]` / `[ADVISORY]` gated checklists" pointer.
> 5. [MINOR] `test_sync_codex_agents.py` only asserts regenerator determinism (`test_committed_direct_mode_refs_match_generator`); it does not guard against dispatch-only language leaking into direct-mode output. The `--check` pass and five-test suite report "PASS" today despite the MAJOR regressions in items 1-2. Consider (future task, not blocking) adding a smoke assertion that the direct-mode outputs do not contain strings like "first dispatch", "re-dispatch prompt", "Worktree: field", or "in the dispatch" (outside of the opening "no dispatch prompt" preface).
>    → orchestrator: deferred — reviewer itself flagged as "future task, not blocking." The regression in items 1-2 exists because the cleanup helpers were deleted, not because the test suite is weak; fixing items 1-2 restores the correct behavior. Test hardening is a sound follow-up but belongs in a separate task so it can be scoped (which strings to guard, where to assert). Tracked for follow-up after this PR lands.
