...
> **Review notes:**
> 1. [MAJOR] DRY violation — the full `## Sync Map` markdown template is duplicated in two places:
>    - `skills/handoff-doc/references/plan-anatomy.md` lines 160-186 (the ```markdown ... ``` block under `**Format:**` inside `## Sync Map`)
>    - `skills/semantic-merge/references/sync-map-format.md` lines 9-35 (the ```markdown ... ``` block under `## Workflow Sync Map`)
>    Both copies carry the same header fields (Base branch / Pre-sync merge base / Synced base head / Incoming range / Sync commit / Sync review status), the same `### Branch Summary` shape, the same cluster blockquote shape, and the same `Sync review notes` shape — just with different placeholder wording (plan-anatomy uses concrete example SHAs; sync-map-format uses `<SHA>` placeholders). This is exactly the duplication CLAUDE.md §DRY prohibits ("When a piece of content appears in more than one skill, at most one copy is authoritative; the rest are one-line pointers.") and that PLAN.md's own Methodology calls out ("Keep one source of truth per concern"). The review focus in this dispatch also called it out explicitly ("no duplicated Sync Map shape across plan-anatomy.md and sync-map-format.md"). Fix: pick one owner for the template and replace the other with a one-line pointer. The natural split is (a) `semantic-merge/references/sync-map-format.md` owns the format template (it is the mode-reference the sync author/reviewer actually load while writing and verifying the map), and (b) `handoff-doc/references/plan-anatomy.md` §Sync Map carries only the section description, ownership, lifecycle, and placement rules plus a pointer like "Format: see `semantic-merge/references/sync-map-format.md §Workflow Sync Map`." Reverse direction is acceptable if you prefer plan-anatomy as the template owner, but sync-map-format.md must then drop the template and link to plan-anatomy.
>    → implemented: dropped the full markdown template block from `skills/handoff-doc/references/plan-anatomy.md §Sync Map` and replaced it with `**Format:** see semantic-merge/references/sync-map-format.md §Workflow Sync Map.` The section now carries only description, ownership, lifecycle, and placement; `sync-map-format.md §Workflow Sync Map` is the sole template owner.
> 2. [MAJOR] The task-local `**Sync impact:**` field anatomy is also duplicated:
>    - `plan-anatomy.md` §Task-local Sync impact (lines 198-206) defines the field with an example and the "pointer, not a duplicate" framing.
>    - `sync-map-format.md` §Task-Local Sync Impact (lines 39-49) defines the same field with an example and the same "not a second Sync Map" framing.
>    Same DRY violation as item 1; same fix — pick one owner and have the other point at it. Keep them consistent on the removal rule wording (plan-anatomy.md says the orchestrator removes satisfied fields "unless a lasting task assumption still belongs in the task block"; sync-map-format.md says the same thing with slightly different phrasing — consolidating also kills this minor wording drift).
>    → implemented: removed the duplicate example block and removal-rule sentence from `plan-anatomy.md §Task-local Sync impact`; the subsection now points at `sync-map-format.md §Task-Local Sync Impact` for format + removal rule while keeping the one-line description of when and where to add the field.
> 3. [MINOR] Sync-review-notes line format drift between the two duplicated templates. `sync-map-format.md` line 33 renders the hint as `> **Sync review notes:** <present only while REVISE is active>` (the parenthetical sits after the bold label as a plain-text tag). `plan-anatomy.md` line 184 renders it as `> **Sync review notes (present only while REVISE is active):**` (the parenthetical is inside the bold label). Consolidating per items 1-2 fixes this automatically — pick whichever form reads better and drop the other.
>    → implemented: the only surviving template copy is in `sync-map-format.md`; rewrote its sync-review-notes line to the parenthetical-inside-bold form `> **Sync review notes (present only while REVISE is active):**` so only one form remains in the codebase.
> 4. [MINOR] `workflow-sync-reviewer.md` allows the sync reviewer to "create a minimal Sync Map with sync-review notes so the issue is in the handoff record before returning REVISE" when no map exists (line 38). `plan-anatomy.md` §Sync Map Ownership (line 147) says the sync reviewer "edits only `**Sync review status:**` and `> **Sync review notes:**` in the map" — implying the author creates the map and the reviewer only annotates it. The two are consistent in the normal case but the plan-anatomy ownership rule should at minimum acknowledge the reviewer-creates-minimal-map exception, or the reviewer reference should route that case back to the sync author. Not a blocker on its own — flag for alignment alongside items 1-2.
>    → implemented: extended the ownership sentence in `plan-anatomy.md §Sync Map` to acknowledge the exception — the sync reviewer may create a minimal Sync Map with sync-review notes when no map exists yet and a finding must be recorded, pointing at `semantic-merge/references/workflow-sync-reviewer.md`.
