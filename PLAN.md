...
- [ ] In `skills/theory-modeling/SKILL.md` Objects & Notation: add a new `[BLOCKING]` item requiring every new symbol to pass two tests, walked symbol by symbol — **Necessity** (removing it makes exposition harder, the result less intuitive, or the algebra materially longer; symbols that abbreviate a one-time expression fail this test) and **Non-duplication** (no symbol already named in `PLAN.md`'s Notation Conventions, the active lemma proof, or a prior derivation in this task names the same object; inheriting from a derivation note does not grant legitimacy if the current proof has a cleaner upstream name available). Failing symbols are removed or replaced. "Defined locally" / "algebraically useful" / "introduced at first use" do not satisfy the gate.
- [ ] In `skills/theory-modeling/SKILL.md` Assumptions: add a sentence stating the same two-test discipline applies to new assumptions (composes with the existing synthesis `[BLOCKING]` rather than replacing it).
- [ ] In `skills/theory-modeling/SKILL.md` Derivations: add a sentence stating the same necessity test applies to new equations and named statements at review time, with no per-equation ledger entry required (the test is enforced via the Derivations gate's existing one-move-per-step + reason-per-move items).
...
- [ ] In `skills/theory-modeling/references/integration.md` §"Derivation discipline preserved through refactoring": add `[BLOCKING]` items that (a) per-task ledger entries survive any refactor (not silently collapsed into prose) and (b) the canonical Notation Conventions table contains only user-confirmed promotions — no orphan entries inserted by implementers post-Task 8.
- [ ] In `skills/handoff-doc/references/results-anatomy.md`: extend the per-task `RESULTS.md` section template with a `**Notation & Assumptions Ledger:**` block that names the four required fields per entry (symbol/assumption, meaning, why it earns its place, near-duplicates rejected) and the "None" convention for empty tasks. Cross-reference `theory-modeling/SKILL.md` for the necessity/non-duplication gate semantics rather than restating them.
- [ ] In `skills/implementation-workflow/SKILL.md` Step 4 completion menu: add a pre-menu hook stating that when the active domain is theory/modeling, the main agent reads each task's RESULTS.md ledger, collects entries not yet in `PLAN.md`'s Notation Conventions table, and surfaces them via `AskUserQuestion` for canonical promotion before presenting the completion-menu disposition. Promotion decisions are logged per `handoff-doc` §User Decisions Log; the table is updated inline-atomic with the decision-log entry.
- [ ] In `agents/reviewer.md` (or theory-modeling SKILL.md reviewer-protocol section, whichever is the cleaner ownership): add reviewer-side discipline — the reviewer walks the ledger entry by entry, applies the two tests to each, and marks `[BLOCKING]` REVISE on any failed entry. Same checklist the implementer self-checks; reviewer's job is verification. Regenerate `skills/using-superRA/references/direct-mode-reviewer.md` and `.codex/agents/superra_reviewer.toml` via `skills/codex-superra-setup/scripts/sync_codex_agents.py` if `agents/reviewer.md` was touched.
- [ ] Sanity-read all edits for internal consistency with the Iron Law and the four-gate structure; `grep -rn "inline-edit.*Notation\|Notation.*inline-edit" skills/` to confirm no stale references to the superseded Task 4 inline-edit rule survive; reconcile any uncommitted draft edits to `SKILL.md` / `references/planning.md` from the 2026-04-25 brainstorming session against the final Task 8 design (keep what aligns, revise what overreaches such as the unilateral extension to "any new equation or named statement", drop what was speculative).
- [ ] Run `tests/check-harness-compatibility.sh` to confirm no regressions; if reviewer.md regenerated artifacts, confirm they pass too. Update `RESULTS.md` Task 8 with final wording and any open caveats.
