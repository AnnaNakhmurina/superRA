...
3. **Read your task source directly from `PLAN.md`.** Read the full task block and the relevant `PLAN.md` header context — the dispatch's "what changed since last dispatch" delta is only a pointer, and paraphrased task descriptions are not authoritative.
